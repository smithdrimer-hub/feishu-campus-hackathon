...
&#x20; - 以 JSON 形式存储，包含 `project\_id`、`type`、`status`、`actors`、`supersedes` 等字段
...
&#x20; "project\_id": "proj\_abc",

&#x20; "chat\_id": "oc\_xxx",         // 飞书群 ID

&#x20; "message\_ids": \["om\_x1"],    // 相关消息 ID 列表
...
&#x20; "actors": \["user\_a", "user\_b"],

&#x20; "status": "open",            // 对决策: "open" | "confirmed" | "reverted"

&#x20;                              // 对任务: "todo" | "in\_progress" | "done"

&#x20; "supersedes": \["mem\_120"],   // 当前条目覆盖/更新了哪些旧条目

&#x20; "created\_at": "2024-04-24T10:00:00Z",    // 事件发生时间

&#x20; "recorded\_at": "2024-04-24T10:05:00Z",   // 写入记忆系统的时间

&#x20; "last\_updated\_at": "2024-04-24T10:05:00Z"
...
&#x20; - `load\_memories(project\_id, chat\_id=None) -> List\[Memory]`

&#x20; - `save\_memory(memory: Memory) -> None`
...
&#x20;  - 形式：一个结构化 JSON，对指定 `project\_id`（和可选 `user\_id`）给出「最有用的上下文包」
...
在指定项目群（`chat\_id`）中，生成/更新一条「项目状态概览」消息，包含：
...
&#x20;  - 用户在群里发送：`@bot 项目状态` 或 `/project\_state`
...
def build\_group\_project\_state(
...
&#x20; "project\_title": "飞书 AI 记忆引擎 Demo",

&#x20; "project\_description": "为多成员项目交接和中断恢复提供长程记忆支持。",

&#x20; "current\_phase": "实现与 Demo 打磨",

&#x20; "last\_major\_update\_at": "2024-04-27T09:00:00Z",
...
&#x20;     "last\_discussed\_at": "2024-04-26T12:00:00Z"
...
&#x20;     "decided\_at": "2024-04-25T20:00:00Z"
...
&#x20;  def render\_group\_state\_panel\_text(state: dict) -> str:

&#x20;      """

&#x20;      把 build\_group\_project\_state 的结果渲染为文本/Markdown。
...
\- 在项目群里：`@bot 我的现在状态` 或 `/my\_state`

\- 或在私聊里：`/my\_state proj\_abc`
...
def build\_personal\_work\_context(

&#x20;   user\_id: str,

&#x20;   project\_id: str | None = None,

&#x20;   chat\_id: str | None = None
...
&#x20;   - 如果只给 chat\_id，尝试推断对应 project\_id；

&#x20;   - 如果给了 project\_id，则以 project\_id 为主。
...
&#x20; "role\_in\_project": "Product \& Research",
...
&#x20; "my\_recent\_decisions\_involved": \[
...
def render\_personal\_context\_text(ctx: dict) -> str:
...
def build\_agent\_context\_pack(

&#x20;   project\_id: str,

&#x20;   user\_id: str | None = None,

&#x20;   max\_items\_per\_section: int = 20
...
&#x20;     "sent\_at": "2024-04-25T19:50:00Z",

&#x20;     "text": "lark-cli 现在能直接做到这些 action，我先搭一个 V1.6 出来……",

&#x20;     "tags": \["engineering\_plan", "feasibility"]
...
\- `recent\_discussion\_snippets` 可以通过：
...
&#x20;  - 参数：`--chat\_id` 或从当前会话环境读取  

&#x20;  - 行为：

&#x20;    - 推断/获取对应 `project\_id`

&#x20;    - 调用 `build\_group\_project\_state` + `render\_group\_state\_panel\_text`
...
&#x20;  - 参数：`--user\_id`、`--project\_id?`、`--chat\_id?`  

&#x20;  - 行为：

&#x20;    - 调用 `build\_personal\_work\_context` + `render\_personal\_context\_text`

&#x20;    - 发送到对应聊天（群里 @ 用户 或 私聊）



3\. `agent\_context\_pack`（供调试/后续接链路）  

&#x20;  - 参数：`--project\_id`、`--user\_id?`  
...
\- 如果你对 `project\_id` 推断有简化假设（例如：一个群只对应一个项目），请写清楚
...
&#x20;  - 在一个测试群中执行 `project\_state`，至少能生成一条包含：
...
&#x20;  - 对给定 `user\_id` + `project\_id`，能返回 JSON 结构（即使某些字段为空列表）
...
&#x20;  - 调用 `agent\_context\_pack` 指令，能得到结构化 JSON，字段名稳定
...
&#x20; - `build\_group\_project\_state`

&#x20; - `render\_group\_state\_panel\_text`

&#x20; - `build\_personal\_work\_context`

&#x20; - `render\_personal\_context\_text`

&#x20; - `build\_agent\_context\_pack`
